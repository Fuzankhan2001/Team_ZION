...
Includes: auto-reconnect, input validation, stats printing.
"""
...
def handle_event(event):
    ...
    if not isinstance(event, dict):
        print("Invalid event: not a dictionary")
        return

    if "facility_id" not in event or "event_type" not in event:
        print("Invalid event: missing required fields")
        return
    ...
    try:
        ...
        if event["event_type"] == "BED_OCCUPANCY":
            ...
            if occupancy_state == "OCCUPIED":
                ...
                print(f"  Bed occupied at {facility} (Total: {state['beds_occupied']})")

            elif occupancy_state == "FREE":
                if state["beds_occupied"] > 0:
                    state["beds_occupied"] -= 1
                    delta["beds_occupied"] = -1
                    print(f"  Bed freed at {facility} (Total: {state['beds_occupied']})")
                else:
                    print(f"  Bed free event ignored - already at 0")
        ...
        # -------- VENTILATOR STATUS --------
            ...
            if status == "IN_USE":
                ...
                if state["ventilators_in_use"] > 0:
                    ...
                    print(f"  Ventilator free event ignored - already at 0")
